...
export class ChargilyClient {
  ...
  private async request<P, R>(
    ...
    try {
      switch (method) {
        case "GET":
          response = (await axios.get<R>(url, { headers })).data;
          break;
        case "POST":
          response = (await axios.post<R>(url, body, { headers })).data;
          break;
        case "DELETE":
          response = (await axios.delete<R>(url, { headers, data: body })).data;
          break;
      }
  ...
  public async updateCustomer(
    ...
    return this.request(`customers/${customer_id}`, 'POST', update_data);
  }
